...
function drawExcel($spreadsheet)
{
    ...
    for ($i = 0; $i < $totData; $i++)
    {
        ...
        $spreadsheet->getActiveSheet()
			...
			->setCellValue('DJ'.$pos, date('d-m-Y', strtotime($data[$i]['DOKTUPTG'])))
			// ->setCellValue('DK'.$pos, $data[$i]['TANGGAL BERANGKAT'])
			// ->setCellValue('DL'.$pos, $data[$i]['TANGGAL BILLING'])
			->setCellValue('DM'.$pos, date('d-m-Y', strtotime($data[$i]['BC23TG'])))
   ...
   //      	->setCellValue('A'.$pos, $data[$i]['CAR'])
    ...
    for ($i = 0; $i < $totData; $i++)
    {
        ...
        $spreadsheet->getActiveSheet()
			...
			->setCellValue('F'.$pos, date('d-m-Y', strtotime($data[$i]['DOKTG'])))
